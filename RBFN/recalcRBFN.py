    ...
    def fit(self,X,Y, penalty=None, numNEvals=None, kfolds=None, testSize=None):
        ...
        if kfolds is not None:
            if numNEvals is None:
                numNEvals=np.inf
            optimizeAcrossPenalty(self, X, Y, numNEvals=numNEvals, kfolds=kfolds, testSize=testSize)
        elif numNEvals is not None or penalty is not None:
            if numNEvals is None:
                numNEvals=np.inf
            optimizeRBFNwithVariableLayers(self, X, Y, numEvals=numNEvals, penalty=penalty)
        else:
            optimizeRBFN(self, X,Y)
...
def optimizeRBFN(toOperateOn, X, Y,penalty=0):
    ...
    def minimizationFunction(optVars):
        alphas,centers, sigmas = breakAndSetFromOptVect(toOperateOn,optVars, (n,m))
    ...
    breakAndSetFromOptVect(toOperateOn,optRes.x, (n, m))
    return optRes
...
def optimizeRBFNwithVariableLayers(toOperateOn, X, Y, numEvals=np.inf, penalty=0, returnN=False):
    ...
    """
    if penalty == 0:
        return optimizeRBFN(toOperateOn,X,Y,penalty=penalty) # obviously, will optimize for higher N. This is training data after all
    n=toOperateOn.hidden_shape
    if len(X.shape)==1:
        m=1
    else:
    ...
    def evalForN(thisN):
        ...
        optRes=optimizeRBFN(toOperateOn,X,Y, penalty=penalty)
        return optRes
    ...
    errs=list((lambda x: x.fun, optResArray))
    minErrIndx=errs.index(min(errs))
    bestOptRes=optResArray[minErrIndx]
    bestN=nGrid[minErrIndx]
    breakAndSetFromOptVect(toOperateOn, bestOptRes.x, (bestN,m))
    if returnN:
...
def optimizeAcrossPenalty(toOperateOn, X,Y, numNEvals=np.inf, kfolds=5, testSize=None, returnLambda=False):
    ...
    for trnIndx, testIndx in kvalList:
        trnX=X[trnIndx,:]; trnY=Y[trnIndx]
        testX=X[testIndx,:]; testY=Y[testIndx]
        accum.append(((trnX,trnY), (testX, testY)))
    def minimizationFunction(lamb):
        thisError=np.zeros(kfolds)
        for i,fold in enumerate(accum):
            optRes=optimizeRBFNwithVariableLayers(toOperateOn, fold[0][0], fold[0][1], numEvals=numNEvals, penalty=lamb)
            ...
            thisError[i]=np.linalg.norm(toOperateOn.predict(fold[1][0]) - fold[1][1])
    ...
    errs=[]; resAccum=[]
    for fold in kvalList:
        optRes, optN=optimizeRBFNwithVariableLayers(toOperateOn, fold[0][0], fold[0][1], numEvals=numNEvals, penalty=lambOptRes.x, returnN=True)
        # breakAndSetFromOptVect(toOperateOn,optRes.x) # currently unnecessary, but might make an ok safeguard if things change in the future and not set to best value at end
        errs.append(np.linalg.norm(toOperateOn.predict(fold[1][0]), fold[1][1]))
        resAccum.append((optRes, optN))
    minErrIndx=errs.index(min(errs))
    bestOptRes=resAccum[minErrIndx]
    breakAndSetFromOptVect(toOperateOn, bestOptRes[0].x, (bestOptRes[1],m))
    if returnLambda:
